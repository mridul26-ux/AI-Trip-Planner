...
def generate_itinerary_with_llm(city: str, days: int, budget: str, interests: str):
    ...
    CRITICAL INSTRUCTION: Ensure a full, rich schedule (at least 3-4 distinct activities per day). You MUST prioritize scheduling the most globally famous, iconic, and highly-recommended "must-see" bucket-list landmarks globally famous to {city} FIRST (early in the trip/day). Then, fill the remaining daily slots with phenomenal, highly-rated cultural highlights and renowned local spots that are slightly less famous but still top-tier. Do NOT EVER use generic placeholders, and NEVER hallucinate false locations. DO NOT suggest individual prices.
